...
def extract_lead_details(
    ...
    -- 3b. Detect if the lead already existed in Sperant BEFORE the current period
    --     (any interaction in ANY project prior to the first day of this month).
    --     Used to classify: 'nuevo' vs 'recaptado'
    existia_antes AS (
        SELECT DISTINCT i2.cliente_id
        FROM tuna.interacciones i2
        INNER JOIN meta_leads ml ON ml.cliente_id = i2.cliente_id
        WHERE i2.fecha_creacion < '{year:04d}-{month:02d}-01'
    ),
    ...
    SELECT
        ...
        ml.utm_term,
        pa.asesor_nombre,
        CASE WHEN ea.cliente_id IS NOT NULL THEN 'recaptado' ELSE 'nuevo' END AS tipo_lead
    FROM meta_leads ml
    LEFT JOIN datos_cliente         dc ON dc.cliente_id = ml.cliente_id
    LEFT JOIN creacion_sperant     cs ON cs.cliente_id = ml.cliente_id
    LEFT JOIN existia_antes        ea ON ea.cliente_id = ml.cliente_id
    ...
    for r in rows:
        ...
        # 19:utm_term, 20:asesor_nombre, 21:tipo_lead
        ...
        results.append({
            ...
            "utm_term":               r[19],
            "asesor_nombre":          r[20],
            "tipo_lead":              r[21],   # 'nuevo' | 'recaptado'
        })
...
def compute_kpis(
    ...
    # Classify leads by tipo_lead
    total_nuevos      = sum(1 for l in leads if l.get("tipo_lead") == "nuevo")
    total_recaptados  = sum(1 for l in leads if l.get("tipo_lead") == "recaptado")
    # total_creados: backward-compat — leads that have fecha_creacion_sperant (= nuevos created in CRM)
